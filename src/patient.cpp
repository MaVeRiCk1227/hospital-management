...
void patient::saveMap()
{
    ...
    f << "patientId,password,firstName,lastName,gender,age,mobNumber,address,height,weight,wasHospitalized?,bedID,stillAlive(ifHospitalized)?\n";
    for (auto i : hospital::patientsList){
        if(i.second.id==0)continue;
        f << i.second.id << "," <<i.second.password<<","<< i.second.firstName << "," << i.second.lastName << "," << i.second.gender
          << "," << i.second.age << "," << i.second.mobNumber << "," << i.second.add.addToStr()
          << "," << i.second.height << "," << i.second.weight << ","
          << (i.second.hospitalized ? 'Y' : 'N') << "," <<(i.second.bedID)<<","<< (i.second.alive ? 'Y' : 'N') << endl;
    }
    f.close();
...
void patient::printDetails()
{
    ...
    if(hospitalized)cout << "Bed ID          : " << bedID << "\n";
...
void patient::printDetailsFromHistory(string extraDetails)
{
    ...
    if (extraDetails == "")
    {
        ...
        while (getline(f >> ws, temp))
        {
            ...
            //creating a string stream object to read from string 'temp';
            stringstream s(temp);
            string s3, s4, s6, s7, s8, s9, s10,s11;
            //reading from the string stream object 's';
            getline(s, p.firstName, ',');
            getline(s, p.lastName, ',');
            getline(s, s3, ',');
...
void patient::getDetails(int rec)
{
    ...
    if (opt == 1)
    {
        ...
        if (hospital::patientsList.find(reqId) != hospital::patientsList.end()){
            ...
            string pw;
            cin>>pw;
            if(pw!=hospital::patientsList[reqId].password){
                cout<<"Wrong password.\n";
                return;
            }
            cout<<"\nCorrect password!\n";
...
void patient::reportADeath()
{
    ...
    if (!hospital::patientsList[id].hospitalized)
        ...
        hospital::patientsList[id].hospitalized=0;
        hospital::bedsList[hospital::patientsList[id].bedID].occupied=0;
    }
    string s, temp, corrected;
    stringstream str;
    fstream f, fout;
