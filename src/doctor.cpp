...
void doctor::saveMap()
{
    ...
    f << "doctorId,password,firstName,lastName,gender,age,mobNumber,address,type,appointmentsBooked\n";
...
void doctor::getDetails(int rec)
{
    ...
    if (opt == 1)
    {
        ...
        if (hospital::doctorsList.find(reqId) != hospital::doctorsList.end()){
            ...
            string pw;
            cin>>pw;
            if(pw!=hospital::doctorsList[reqId].password){
                cout<<"Wrong password.\n";
                return;
            }
            cout<<"\nCorrect password!\n";
