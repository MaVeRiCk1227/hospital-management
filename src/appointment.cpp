...
void appointment::saveMap()
{
    ...
    f << "appointmentId,date(YYYYMMDD),doctorId,patientId,startTime(in 24-hr format),if_online\n";
    for (auto i : hospital::appointmentsList)
        f << i.second.id << "," << yyyymmdd << "," << i.second.D.id << "," << i.second.P.id
          << "," << i.second.hh <<','<<i.second.if_online<< endl;
    f.close();
...
void appointment::book()
{
    ...
    cout<<"Here are all the idle doctors:\n";
...
void appointment::book_online()
{
    if (hospital::online_appointmentsList.size() >= 8 * hospital::doctorsList.size())
    {
        cout << "\n\nSorry, no doctor is available for online_appointment today!\n\n";
        return;
    }
    cout << "\n\nIs the patient already registered (Y : Yes || N : No)?\n";
    char ans;
    ...
    while (ans == 'Y')
    {
        ...
        {
            cout << "Sorry, selected doctor has no free online_slot left for the day!\n";
            cout << "Search again (Y : Yes || N : No)?\n";
            cin >> ans;
            while (ans != 'Y' && ans != 'N')
            {
    ...
    if (hospital::online_appointmentsList.rbegin() != hospital::online_appointmentsList.rend())
        id = ((hospital::appointmentsList.rbegin())->first) + 1;
    else
        id = 1;
    hh = 9 + D.appointmentsBooked;
